...
-   В кластере Kubernetes должен быть настроен `StorageClass` для автоматического выделения `PersistentVolume`, если используется кэширование.
...
## Практический пример: Настройка и проверка кэширования (PVC)

Роль `install_jenkins` настроена на использование `PersistentVolumeClaim` (PVC) для кэширования, но она **не создает** этот PVC самостоятельно. Его необходимо создать в Kubernetes вручную **один раз**.

Этот гайд показывает, как создать PV и PVC для тестового окружения, используя `hostPath`.

**ВНИМАНИЕ:** Метод `hostPath` подходит **только для разработки** на однонодовом кластере. В production используйте подходящий для вашего окружения `StorageClass`.
